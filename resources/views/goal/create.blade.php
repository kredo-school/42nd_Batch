...
<head>
  ...
  <style>
    ...
    .form-input-custom:focus { border-color:var(--purple); }
    ...
    .form-select-custom:focus { border-color:var(--purple); }
    .form-textarea-custom { width:100%; padding:11px 14px; border-radius:10px; border:1.5px solid rgba(28,26,23,.1); background:white; font-family:'DM Sans',sans-serif; font-size:14px; color:var(--ink); outline:none; resize:vertical; min-height:90px; transition:border-color .2s; }
    .form-textarea-custom:focus { border-color:var(--purple); }
    .cat-card { border-radius:12px; padding:14px; text-align:center; cursor:pointer; border:2px solid transparent; transition:all .15s; background:white; }
    .cat-card:hover { transform:translateY(-2px); }
    .cat-card.selected { border-color:var(--purple); background:var(--purple-pale); }
    .cat-icon { font-size:28px; margin-bottom:6px; }
    .cat-name { font-size:12px; font-weight:600; color:var(--ink); }
    .btn-save { background:var(--purple); color:white; border:none; border-radius:12px; padding:14px 36px; font-size:14px; font-weight:600; cursor:pointer; transition:.18s; font-family:'DM Sans',sans-serif; }
    .btn-save:hover { background:#7A5A9E; transform:translateY(-1px); }
...
<body x-data="{ category: '{{ old('category', 'reflection') }}' }">
  ...
  <div class="main-content">
    <div class="topbar d-flex align-items-center px-4">
      ...
      <span class="text-muted" style="font-size:12px;"><a href="{{ route('goal.index') }}" style="color:var(--ink-muted);text-decoration:none;">Goal Tracking</a></span>
      <span class="text-muted mx-2" style="opacity:.4;">›</span>
      <span style="font-size:15px;font-weight:600;color:var(--ink);">Set New Goal</span>
    ...
    <div class="p-4">
      <div class="hero-card">
        ...
        <div class="hero-eyebrow">New Goal</div>
        ...
        <div class="row g-4">
          <div class="col-8">
            <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
              ...
              @error('category')<div style="font-size:12px;color:var(--rose);margin-bottom:8px;">{{ $message }}</div>@enderror
              <div class="row g-2">
                @foreach([['reflection','✍️','Reflection'],['exercise','🏃','Exercise'],['reading','📚','Reading'],['study','💻','Study'],['other','🎵','Other']] as [$val,$icon,$label])
                <div class="col">
                  <div class="cat-card" :class="{ selected: category === '{{ $val }}' }" @click="category = '{{ $val }}'">
            ...
            <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
              <div class="mb-3">
                ...
                <input type="text" name="title" class="form-input-custom" placeholder="e.g. Write reflection every day" value="{{ old('title') }}" required>
                @error('title')<div style="font-size:12px;color:var(--rose);margin-top:4px;">{{ $message }}</div>@enderror
              </div>
              <div class="row g-3 mb-3">
                ...
                <div class="col-3">
                  ...
                  <input type="number" name="target" class="form-input-custom" placeholder="30" min="1" value="{{ old('target') }}" required>
                  @error('target')<div style="font-size:12px;color:var(--rose);margin-top:4px;">{{ $message }}</div>@enderror
                </div>
                <div class="col-3">
                  <label class="form-label-custom">Unit</label>
                  <input type="text" name="unit" class="form-input-custom" placeholder="days / times" value="{{ old('unit') }}">
                </div>
              </div>
              <div>
                ...
                <textarea name="note" class="form-textarea-custom" placeholder="Why is this goal important to you?">{{ old('note') }}</textarea>
              </div>
            </div>

            <div class="d-flex gap-3 align-items-center">
              <button type="submit" class="btn-save">🎯 Save Goal</button>
              <a href="{{ route('goal.index') }}" class="btn-cancel">Cancel</a>
            </div>
          </div>

          <div class="col-4">
            <div style="font-size:11px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--ink-muted);margin-bottom:12px;">Goal Setting Tips 💡</div>
            <div class="card border-0 shadow-sm rounded-4 p-4 mb-3">
              <div class="d-flex flex-column gap-3" style="font-size:13px;color:var(--ink-muted);line-height:1.65;">
                <div class="d-flex gap-2"><span style="color:var(--purple);">🎯</span><span>Set specific, measurable goals with clear numbers.</span></div>
                ...
                <div class="d-flex gap-2"><span style="color:var(--blue);">📊</span><span>Track progress daily to stay motivated and accountable.</span></div>
              </div>
            </div>
          </div>
        </div>
