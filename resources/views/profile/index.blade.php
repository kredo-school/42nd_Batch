...
<head>
  ...
  <style>
    ...
    .profile-banner { background:linear-gradient(140deg,#2A2420 0%,#3D3228 60%,#4A3C2E 100%); border-radius:20px; padding:36px; position:relative; overflow:hidden; color:white; margin-bottom:24px; }
    ...
    .profile-avatar-lg { width:80px; height:80px; border-radius:50%; background:linear-gradient(135deg,var(--amber),var(--rose)); display:flex; align-items:center; justify-content:center; font-family:'DM Serif Display',serif; font-size:32px; color:white; flex-shrink:0; }
    ...
    .stat-badge { background:rgba(255,255,255,.08); border:1px solid rgba(255,255,255,.12); border-radius:12px; padding:14px; text-align:center; }
    .stat-val { font-family:'DM Serif Display',serif; font-size:26px; color:white; line-height:1; margin-bottom:4px; }
    ...
    .btn-cancel-sm { background:transparent; color:var(--ink-muted); border:1.5px solid rgba(28,26,23,.15); border-radius:10px; padding:10px 20px; font-size:13px; font-weight:500; cursor:pointer; font-family:'DM Sans',sans-serif; }
  </style>
</head>
<body>
    ...
    <div class="p-4">
      ...
      <div class="profile-banner">
        <div class="d-flex align-items-center gap-4 position-relative mb-4">
          ...
          <button class="btn fw-semibold" style="background:rgba(255,255,255,.1);color:white;border:1px solid rgba(255,255,255,.2);border-radius:10px;font-size:13px;" onclick="document.getElementById('edit-form').scrollIntoView({behavior:'smooth'})">✏️ Edit Profile</button>
        </div>
      ...
      <div class="row justify-content-center">
        <div class="col-8">
          ...
          <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
            ...
            <div class="info-row"><span class="info-label">Member Since</span><span class="info-value">{{ $user->created_at->format('F j, Y') }}</span></div>
            <div class="info-row"><span class="info-label">Total Reflections</span><span class="info-value">{{ $totalDays }} days</span></div>
            <div class="info-row"><span class="info-label">Total Activities</span><span class="info-value">{{ $totalActs }} logged</span></div>
            <div class="info-row"><span class="info-label">Account Status</span><span class="badge rounded-pill px-3 py-1" style="background:var(--sage-pale);color:var(--sage);font-size:12px;">Active</span></div>
          </div>
          ...
          <div class="card border-0 shadow-sm rounded-4 p-4">
            <form method="POST" action="{{ route('profile.update') }}">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="name" value="{{ old('name', $user->name) }}" class="form-input-custom" required>
                @error('name')<div style="font-size:12px;color:var(--rose);margin-top:4px;">{{ $message }}</div>@enderror
              </div>
              <div class="mb-3">
                <label class="form-label-custom">Email</label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="form-input-custom" required>
